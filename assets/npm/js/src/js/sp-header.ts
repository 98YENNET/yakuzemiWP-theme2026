...
export function initSpHeader(): void {
  ...
  window.addEventListener('scroll', () => {
    ...
    const menuOpen = document.documentElement.classList.contains('sb-active-right');
...
export function initSpDrawer(): void {
  ...
  const menuBtn = document.querySelector<HTMLElement>('.sb-toggle-right.mb-menu-button');
  const header = document.querySelector<HTMLElement>('body.aft26 header.hidePC');
